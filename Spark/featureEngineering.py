...
if __name__== "__main__":
	...
	def convertToFeatures(inputTuple, customerList):
		...
		if max == min:
			trendUp = 0.0
			trendDown = 0.0
		dataSetCount = sum(custCounter.values())
		...
		for customer in customerList:
			...
			index += 1
		return(ip,max,min,avg,trendUp,trendDown,trueCount,dataSetCount,
	...
	completeDf = spark.createDataFrame(finalFeatures, ['ip','maxScore','minScore','avgScore','trendUp','trendDown','trueCount','dataSetCount','mostCommonCustomerHit','totalCustomersHit'] + customerList )
