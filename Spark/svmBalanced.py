...
if __name__ == "__main__":
    ...
    trainAbove5 = train.filter(lambda x: x.label ==1.0)
    trainBelow5 = train.filter(lambda x: x.label ==0.0)
    #ratio is >5 / <5 not  coding it here cause collecting takes time.
    ratio = 0.44227
    trainBelow5 = trainBelow5.sample(False,ratio)
    train = trainBelow5.union(trainAbove5)
    train.cache()
    ...
    listToOutputRDD = sc.parallelize(listToOutput, 1)\
        .saveAsTextFile("s3://daen-cyber/models/no5sSvmResultsBal0")

    model.save(sc, "s3://daen-cyber/models/no5sSvmModelBal0")
